...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[PAC] = list((x, y) for x in range(X_RANGE) for y in range(Y_RANGE))
    max_manhattan_dist = (X_RANGE-1) + (Y_RANGE -1) + MAX_NOISE + 1
    variableDomainsDict[GHOST0] = list((x, y) for x in range(X_RANGE) for y in range(Y_RANGE))
    variableDomainsDict[GHOST1] = list((x, y) for x in range(X_RANGE) for y in range(Y_RANGE))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for eliminated_var in eliminationOrder:
            # reassign like inferencebyenumeration does
            # join first
            currentFactorsList, joined_factor = joinFactorsByVariable(currentFactorsList, eliminated_var)
            if len(joined_factor.unconditionedVariables()) == 1:
                #do nothing
                pass
            else:
                # then eliminate
                eliminated_joined_factors = eliminate(joined_factor, eliminated_var)
                currentFactorsList.append(eliminated_joined_factors)
        fullJoint = joinFactors(currentFactorsList)
        full_joint_normalized = normalize(fullJoint)
        return full_joint_normalized
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jail_position:
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            curr_belief = self.beliefs[position]
            observation_prob = self.getObservationProb(observation, gameState.getPacmanPosition(), position, self.getJailPosition())
            new_beleif = observation_prob * curr_belief
            self.beliefs[position] = new_beleif
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
